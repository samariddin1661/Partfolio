  ...
  return (
    <nav className='containerr fixed backdrop-blur w-full h-[80px]  flex justify-between items-center'>
      <div className=' md:text-[25px] text-[14px] font-bold font-serif '>
        <p className='flex items-end '><p className='flex items-end text-blue-800 md:text-[30px] text-[18px]'>S</p>am <p className='flex items-end text-blue-800 md:text-[30px] text-[18px]'>AR</p> iddin.</p>
      </div>
      
        <ul className={`flex right-0 top-[80px] h-[100vh] py-4 flex-col gap-12 text-center text-[#6b7688] absolute bg-[white]   w-[70%] font-[600] uppercase md:flex-row transition-[0.5s] ${toggle ? 'right-0' : 'right-[-100vh]'}   md:bg-[transparent] md:w-auto md:h-auto md:static`}>
